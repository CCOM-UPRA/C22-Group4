...
def login():
    ...
    return logincontroller(email=email, password=passcode, flag=False)
...
def registerinfo():
    ...
    if pass1 == pass2:
        ...
        email = email
        passcode = pass1
        logincontroller(email=email, password=passcode, flag=flag)
...
def filter():
    ...
    #filtered_products = [p for p in products if p['brand'] == brand_filter] if brand_filter else products
