...
# Done in array instead of dictionaries to portray the differences
                ...
                #ID   Name     Brand  Description     Category   Price    Stock   PNG   Color
...
def getPricesModel(select):
    prices = productList[select][5]
    return prices

def getBrandsModel():
    # Simulating grabbing these filters via SQL from the database
    brands = ["Ware", "Higgins", "Kaytee", "Tirifer", "Tetra", "Zeny", "Purina", "MeoHui", "Best Pet Supplies"]
    return brands

def getColorsModel():
    colors = ["Blue", "Teal", "White", "Other", "N/A", "Grey"]
    return colors

def getCategoriesModel():
    categories = ["Enclosures","Food","Toys"]
    return categories